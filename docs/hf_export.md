...
clm_p1.kosmos (@corpus, SSOT)            HF dataset (dancinlab/<id>)
  ...
  member = ref "*.limen" sha256= ─→   data_files (sha256 무결성 carry)
...
| inline `@anchor` member | shard로 packing 후 동일 처리(`spec/limen.md`) — 또는 소규모면 inline JSONL로 emit |
...
2. shard(.limen) → HF data_files 경로로 배치 (대용량은 HF/R2, manifest만 git).
...
- ⏳ `.limen` 디코드→HF rows 변환: `spec/limen.md` 바이너리 리더 구현 후 (2.x).
